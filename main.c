...
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
#define FRAME_TIMEOUT 5000    // in ms, adjust to your baudrate
        ...
        switch (rx_state)
        {
            case WAIT_START:
                if (b == 0xAA)
                {
                    ...
                    rx_index = 1;  // Start filling from index 1
                }
                break;

            case WAIT_LENGTH:
                rx_buffer[rx_index++] = b;  // Store length byte
                expected_length = b;
                if (expected_length > 0 && expected_length < (sizeof(rx_buffer) - 3))  // ensure space
                {
                    rx_state = RECEIVE_DATA;
                }
                else
                {
                    rx_state = WAIT_START;  // Reset if invalid
                    rx_index = 0;
                }
                break;
            ...
            case RECEIVE_DATA:
                ...
                if (rx_index >= (expected_length + 2))  // +2 for start + length
                {
                    rx_state = WAIT_STOP;
                }
                break;
            ...
            case WAIT_STOP:
                if (b == 0x55)
                {
                    rx_buffer[rx_index++] = b;  // Store stop byte
